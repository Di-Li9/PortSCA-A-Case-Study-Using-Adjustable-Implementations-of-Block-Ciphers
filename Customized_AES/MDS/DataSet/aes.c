...
void mixColumns_Reconfigurable (u8 state[16],u8 MDS[4]) //����MDS������MDS
{
  int i;
  u8 out[16];
  for (i = 0; i < 4; i++) {
    out[4*i] = gMul(MDS[0], state[4*i]) ^ gMul(MDS[1], state[4*i + 1]) ^ gMul(MDS[2], state[4*i + 2]) ^ gMul(MDS[3], state[4*i + 3]);
